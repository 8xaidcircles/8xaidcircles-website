          ...
          <div className="max-w-6xl mx-auto px-4 py-10">
            ...
            <ul className="space-y-8">
              {news.map((item) => (
                <li
                  key={item.id}
                  className="bg-[#f6f8fa] rounded-2xl px-8 py-8 flex flex-col cursor-pointer transition-shadow hover:shadow-md"
                  onClick={() => handleClick(item.id)}
                >
                  <div className="flex flex-col md:flex-row md:items-center md:space-x-8 w-full">
                    <span className="text-lg md:text-xl font-semibold text-[#222] w-32 mb-2 md:mb-0">{item.publishedAt.slice(0, 10).replace(/-/g, ".")}</span>
                    ...
                    <span className="text-lg md:text-xl text-[#222] font-medium flex-1">{item.title}</span>
                  </div>
                </li>
              ))}
            </ul>
