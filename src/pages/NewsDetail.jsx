    ...
    <Layout>
      ...
      font-['Open_Sans']">
        <section className="min-h-screen flex flex-col justify-center items-center py-16 sm:py-24 md:py-32 opacity-0" ref={newsRef}>
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 md:py-10 bg-white rounded-xl sm:rounded-2xl shadow-lg">
           
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-6 sm:mb-8 opacity-0">
                <span className="
            ...
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4 lg:space-x-8 w-full mb-6 sm:mb-8">
              <span className="text-base sm:text-lg md:text-xl font-semibold text-[#222] w-24 sm:w-32 mb-2 md:mb-0">{news.publishedAt.slice(0, 10).replace(/-/g, ".")}</span>
            ...
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#222] mb-6 sm:mb-8">{news.title}</h2>
            <div className="text-gray-700 text-sm sm:text-base md:text-lg" dangerouslySetInnerHTML={{ __html: news.content }} />
            <button
              onClick={() => navigate("/news")}
              className="mt-6 sm:mt-8 px-4 sm:px-6 py-2 sm:py-3 bg-[#1da1f2] text-white rounded-full hover:bg-[#1a8cd8] transition-colors text-sm sm:text-base"
            >
              ニュース一覧に戻る
            </button>
          </div>
